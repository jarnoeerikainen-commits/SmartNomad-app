...
serve(async (req) => {
  ...
  try {
    ...
    const proofCore = {
      ...
      request_category: cleanText(body.request_category || "general_ai", 120),
      status,
      route,
      directors: asArray(body.directors),
      steps: asArray(body.steps),
      sources: asArray(body.sources),
      tools_actions: asArray(body.tools_actions),
      data_sources: asArray(body.data_sources),
      ...
      confidence_status: cleanText(body.confidence_status || (body.error ? "failed" : "unverified"), 40),
      human_approval_state: cleanText(body.human_approval_state || "not_required", 40),
      human_approval_actor: cleanText(body.human_approval_actor, 120) || null,
      human_approval_at: body.human_approval_at || null,
      escalation_type: cleanText(body.escalation_type, 80) || null,
      escalation_reason: cleanText(body.escalation_reason, 500) || null,
      retention_until: body.retention_until || new Date(Date.now() + 6 * 365 * 24 * 60 * 60 * 1000).toISOString(),
