...
serve(async (req) => {
  ...
  try {
    ...
    const userContext = body.userContext && typeof body.userContext === 'object' ? {
      ...
      lifestyle: (body.userContext.lifestyle && typeof body.userContext.lifestyle === 'object') ? body.userContext.lifestyle : undefined,
    } : undefined;
...
${userContext?.language && userContext.language !== 'en' ? `**🌍 LANGUAGE: The user's app language is "${userContext.language}". You MUST respond ENTIRELY in this language. All legal advice, emergency steps, rights information — everything in the user's language. Legal terms can include the original + translation.**` : ''}`;
    ...
    const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: buildScopeGuard('legal') + withTruthProtocol(systemPrompt) },
          ...messages
        ],
        stream: true,
      }),
