...
export function getJetSearchAIContext(flights: CharterFlight[], homeAirport: string, personaId?: string): string {
  ...
  ctx += `4. **COMMERCIAL FLIGHTS FIRST.** Always lead with commercial flight search links (Skyscanner and Kayak). Private jet options are a BONUS — only add them AFTER commercial results and ONLY if a matching route exists in the database.\n`;
