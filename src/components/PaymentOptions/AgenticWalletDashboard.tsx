      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Agent-assisted payments with explicit controls
          </p>
        </div>
        <Badge className="bg-primary/10 text-primary border-primary/30 gap-1 text-xs">
          <Cpu className="h-3 w-3" /> Demo controls
        </Badge>
      </div>
            ...
            <Card key={key} className={`p-3 border-2 transition-all ${enabled ? info.borderClass : 'border-muted opacity-60'}`}>
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  toast({ title: v ? `${info.name} demo enabled` : `${info.name} paused` });
                  }}
                />
              </div>
            </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between p-3 rounded-lg border">
                <div>
                   <p className="text-sm font-medium">Delegated booking preview</p>
                   <p className="text-xs text-muted-foreground">Invitation-only mandates; live money movement is not connected</p>
                </div>
                <Switch
              ...
              <div className="p-3 rounded-lg border space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                   Demo mandate limit: ${config.maxAutoPayAmount}. Initial live purchases will still require you to be present.
                </p>
              </div>
              <div className="flex items-center justify-between p-3 rounded-lg border">
                <div>
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    {gr.requireApproval ? (
                      ...
                      <Badge variant="outline" className="gap-1 text-emerald-600 border-emerald-300">
                         <CheckCircle2 className="h-3 w-3" /> Demo rule only
                      </Badge>
                    )}
        ...
        <TabsContent value="vcards" className="mt-4 space-y-4">
          <Card>
            <CardHeader className="pb-3">
              ...
              <p className="text-xs text-muted-foreground">
                 Simulated card references for testing controls. No real card number is created or stored.
              </p>
            </CardHeader>
            <CardContent className="space-y-3">
              {virtualCards.map(vc => {
      ...
      <IssueVirtualCardModal
        ...
        onIssue={(vc) => {
          setVirtualCards(prev => [vc, ...prev]);
           toast({ title: 'Demo card reference created', description: `${vc.network.toUpperCase()} •••${vc.last4} is simulated and cannot be charged.` });
        }}
      />
    </div>
  );
          ...
          <div className="p-3 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800">
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs text-emerald-700 dark:text-emerald-300">
                 Demo only. A live version requires approved tokenization and issuing accounts; no card number is created here.
              </p>
            </div>
          </div>
          <Button onClick={handleSubmit} disabled={!amount || (type === 'merchant-locked' && !merchant)} className="w-full gap-2">
             <CreditCard className="h-4 w-4" /> Create demo reference
          </Button>
        </div>
      </DialogContent>
    </Dialog>
